		...
		switch algoritmo_planificacion
        {
            case FIFO:
				...
				if recibir_confirmacion {                     // funcion tipo int que imita un bool (1=confirmo, 0=no confirmo)
                    ...
                    pop_estado (struct.ColaNew) // saco la cola new
...
func iniciar_planificacion(void){    
...
func _chequear_algoritmo_corto() t_algoritmo {
    if (strings.EqualFold(configCargadito.scheduler_algorithm, "FIFO")) { return FIFO }
    if (strings.EqualFold(configCargadito.scheduler_algorithm, "SJF"))   { return SJF }
    if (strings.EqualFold(configCargadito.scheduler_algorithm, "SRT"))  { return SRT } 

    return ERROR  
}

func _chequear_algoritmo_largo() t_algoritmo {
    if (strings.EqualFold(configCargadito.ready_ingress_algorithm, "FIFO")) { return FIFO }
    if (strings.EqualFold(configCargadito.ready_ingress_algorithm, "PMCP"))   { return PMCP }
...
func pop_estado(Cola *struct.ColaProcesos) *struct.PCB{   ////////////go version 
    MutexCola.Lock()
	    var pcb *struct.PCB
		pcb= Cola[0]
