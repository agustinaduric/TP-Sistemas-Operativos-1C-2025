	...
	for {
		...
		if (len(structs.ColaReady)) == 0 {
			...
			case "SRT":
				...
				var Cola structs.ColaProcesos = OrdenarColaPorSJF()
				pcb_execute = Cola[0]
				pcb_execute.EstimadoRafagaAnt = pcb_execute.EstimadoRafaga
				global.MutexCpuDisponible.Lock()
				var Cpu_disponible structs.CPU_a_kernel = protocolos.Buscar_CPU_libre(pcb_execute)
				...
				if Cpu_disponible.Identificador == "" {
					...
					Cpu_disponible = protocolos.Buscar_CPU_Para_Desalojar(pcb_execute)
...
func limpieza_cola_exit() {
	for {
		...
		if respuesta := protocolos.Enviar_P_Finalizado_memoria(ProcesoExit.PID); respuesta == "OK" {
			...
			global.KernelLogger.Debug("Memoria no avalo la finalizacion del proceso")
		}

	}
}

func OrdenarColaPorPMCP(cola structs.ColaProcesos) structs.ColaProcesos {
	aux := make(structs.ColaProcesos, len(cola))
	copy(aux, cola)

	sort.Slice(aux, func(i, j int) bool {
		if aux[i].Tamanio != aux[j].Tamanio {
			return aux[i].Tamanio < aux[j].Tamanio
		}
